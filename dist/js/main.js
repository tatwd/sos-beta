...
function caroStart() {
    ...
    var ctrls  = document.getElementsByClassName('caro-ctrl');
    ...
    function go() {
        ...
        if(curent >= imgs.length - 2) {  // 去除caro-right的两张图片
            curent = 0;
        }
    ...
    function bk() {
        fadeOut();
        if(curent == 0) {
            curent = imgs.length - 2;   // 去除caro-right的两张图片
        }
        curent--;
        fadeIn();
    }
...
function setLinesStyle(obj, isClicked){
    ...
    // 旋转
    obj[0].style.transform = degs;
    obj[0].style.WebkitTransform = degs;
    obj[0].style.msTransform = degs;
    
    obj[obj.length-1].style.transform = odegs;
    obj[obj.length-1].style.WebkitTransform = odegs;
    obj[obj.length-1].style.msTransform = odegs;
    
    // 过渡
    obj[0].style.transition = tras;
    obj[0].style.WebkitTransition = tras;
    
    obj[obj.length-1].style.transition = tras;
    obj[obj.length-1].style.WebkitTransition = tras;
